			...
			if ($db_key === $k) {
				...
				if (empty($w) || $w == null) {
					...
					$res = $this->Ratemodel->is_valid_platform($k, $w);
                     $users_id = $res->user_id;
                     
                    $this->db->select('*');
            $this->db->from('users');
            $this->db->where('id', $users_id);
            $web_url1 = $this->db->get()->row();
		...
		if (count($_POST) > 0 && $_POST['form_key'] && $_POST['name'] && $_POST['mobile']) {
			...
			if ($q_res === 'not_found') { //invalid data (not found in DB)
				...
				$rData = array(
					...
					'star'        => (int) htmlentities($_POST['starv']),
					'r_food'      => isset($_POST['r_food'])      ? (int) $_POST['r_food']      : null,
					'r_beverages' => isset($_POST['r_beverages']) ? (int) $_POST['r_beverages'] : null,
					'r_order_time'=> isset($_POST['r_order_time'])? (int) $_POST['r_order_time']: null,
					'r_serve_time'=> isset($_POST['r_serve_time'])? (int) $_POST['r_serve_time']: null,
					'r_staff'     => isset($_POST['r_staff'])     ? (int) $_POST['r_staff']     : null,
					'r_restaurant'=> isset($_POST['r_restaurant'])? (int) $_POST['r_restaurant']: null,
					'r_menu'      => isset($_POST['r_menu'])      ? (int) $_POST['r_menu']      : null,
					'r_care'      => isset($_POST['r_care'])      ? (int) $_POST['r_care']      : null,
