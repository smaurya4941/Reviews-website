	...
	public function login()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($validate == FALSE) {
				...
				$this->setFlashMsg('error', lang('wrong_pwd_uname') ? lang('wrong_pwd_uname') : 'Invalid Username or Password');
				redirect('login');
			}
			if ($validate == "inactive_access_by_cmpyAdmin") {
				...
				$this->setFlashMsg('error', lang('acct_deact_by_cmpy_admin') ? lang('acct_deact_by_cmpy_admin') : 'Account deactivated by Company Admin');
				redirect('login');
			}
			if ($validate == "inactive_access") {
				...
				$this->setFlashMsg('error', lang('acct_deact') ? lang('acct_deact') : 'Account deactivated');
				redirect('login');
			}
			...
			if (is_object($validate)) {
				...
				$cmpy = $validate->cmpy;
				$cmpy_logo = isset($validate->cmpyLogo) ? $validate->cmpyLogo : '';
				$sub = $validate->sub;
